...
/*
Bindings required in Pages:
- DB (D1)
- JWT_SECRET
- PASPAY_* (if using checkout/webhook)
*/
...
// --- helpers for table introspection ---
async function tableHasColumn(db, tableName, columnName) {
  try {
    ...
    return rows.some((r) => (r.name || r.column_name) === columnName);
  } catch (e) {
    console.warn('tableHasColumn error', e && e.message);
    return false;
  }
}
...
// list products (apply is_active filter only if column exists)
...
async function storeProductByIdHandler(c) {
  ...
  try {
    ...
    const gallery = normalizeAllResult(galleryRaw).map(g => g.image_url || g.image);
...
async function storeProductBySlugHandler(c) {
  const env = c.env; const slug = c.req.param('slug');
  try {
    const p = await env.DB.prepare(
      `SELECT p.id, p.slug, p.name, p.price, p.description, p.image_url, c.name as category_name, p.digital_content, p.product_type
       FROM products p LEFT JOIN categories c ON p.category_id = c.id
...
async function loginHandler(c) {
  ...
  try { loginSchema.parse(body); } catch (zerr) { return c.json({ error: 'Invalid payload', detail: zerr.errors }, 422); }
  try {
...
// --- admin categories ---
async function adminListCategories(c) { const env = c.env; const raw = await env.DB.prepare("SELECT * FROM categories ORDER BY name ASC").all(); return c.json(normalizeAllResult(raw)); }
async function adminCreateCategory(c) { const env = c.env; const body = c.req.valid('json'); const { results } = await env.DB.prepare("INSERT INTO categories (name, slug) VALUES (?, ?) RETURNING *").bind(body.name, body.slug).all(); return c.json(normalizeAllResult(results)[0], 201); }
async function adminUpdateCategory(c) { const env = c.env; const id = c.req.param('id'); const body = c.req.valid('json'); const { results } = await env.DB.prepare("UPDATE categories SET name = ?, slug = ? WHERE id = ? RETURNING *").bind(body.name, body.slug, id).all(); return c.json(normalizeAllResult(results)[0]); }
async function adminDeleteCategory(c) { const env = c.env; const id = c.req.param('id'); await env.DB.prepare("DELETE FROM categories WHERE id = ?").bind(id).run(); return c.json({ success: true, message: 'Kategori dihapus' }); }

// --- admin products (create/update adapt to slug & is_active existence) ---
...
async function adminCreateProduct(c) {
  ...
  // ensure slug exists / unique: generate if empty
  let slug = (body.slug || '').trim();
  if (!slug) slug = slugify(body.name);
  // ensure uniqueness: if slug exists append numeric suffix
  let candidate = slug;
  let i = 1;
  while (true) {
...
async function adminUpdateProduct(c) {
  ...
  const hasIsActive = await tableHasColumn(env.DB, 'products', 'is_active');
  // handle slug: if provided ensure uniqueness (excluding current id)
  let slug = (body.slug || '').trim();
  if (!slug) slug = slugify(body.name);
  let candidate = slug;
  let i = 1;
...
// admin stock & gallery (unchanged)...
async function adminListStock(c) { const env = c.env; const id = c.req.param('id'); const raw = await env.DB.prepare("SELECT * FROM product_stock_unique WHERE product_id = ? ORDER BY id DESC").bind(id).all(); return c.json(normalizeAllResult(raw)); }
async function adminAddStock(c) { const env = c.env; const id = c.req.param('id'); const body = c.req.valid('json'); const product = await env.DB.prepare("SELECT id FROM products WHERE id = ? AND product_type = 'UNIQUE'").bind(id).first(); if (!product) return c.json({ error: 'Produk tidak ditemukan atau bukan tipe UNIQUE' }, 404); const statements = body.stock_items.map(content => env.DB.prepare("INSERT INTO product_stock_unique (product_id, content, is_sold) VALUES (?, ?, 0)").bind(id, content)); if (statements.length === 0) return c.json({ error: 'Tidak ada stok yang diberikan' }, 400); await env.DB.batch(statements); return c.json({ success: true, message: `${statements.length} item stok ditambahkan` }, 201); }
async function adminDeleteStock(c) { const env = c.env; const stockId = c.req.param('stockId'); const { changes } = await env.DB.prepare("DELETE FROM product_stock_unique WHERE id = ? AND is_sold = 0").bind(stockId).run(); if (changes === 0) return c.json({ error: 'Gagal menghapus stok (mungkin sudah terjual atau tidak ditemukan)' }, 404); return c.json({ success: true, message: 'Stok dihapus' }); }

async function adminListGallery(c) { const env = c.env; const id = c.req.param('id'); const raw = await env.DB.prepare("SELECT * FROM product_images WHERE product_id = ? ORDER BY sort_order ASC").bind(id).all(); return c.json(normalizeAllResult(raw)); }
async function adminSyncGallery(c) { const env = c.env; const id = c.req.param('id'); const body = c.req.valid('json'); const deleteStmt = env.DB.prepare("DELETE FROM product_images WHERE product_id = ?").bind(id); const insertStmts = body.images.map((url, index) => env.DB.prepare("INSERT INTO product_images (product_id, image_url, sort_order) VALUES (?, ?, ?)").bind(id, url, index)); await env.DB.batch([deleteStmt, ...insertStmts]); return c.json({ success: true, message: `Galeri disinkronkan (${insertStmts.length} gambar)` }, 201); }
async function adminDeleteGallery(c) { const env = c.env; const imageId = c.req.param('imageId'); await env.DB.prepare("DELETE FROM product_images WHERE id = ?").bind(imageId).run(); return c.json({ success: true, message: 'Gambar Galeri Dihapus' }); }

// orders & users
async function adminListOrders(c) { const env = c.env; const raw = await env.DB.prepare(`SELECT o.*, p.name as product_name, u.email as user_email FROM orders o LEFT JOIN products p ON o.product_id = p.id LEFT JOIN users u ON o.user_id = u.id ORDER BY o.created_at DESC`).all(); return c.json(normalizeAllResult(raw)); }
async function adminListUsers(c) { const env = c.env; const raw = await env.DB.prepare("SELECT id, email, name, role, status, created_at FROM users ORDER BY created_at DESC").all(); return c.json(normalizeAllResult(raw)); }

// checkout & webhook left unchanged (use earlier implementation)...
