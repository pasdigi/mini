/**
 * Full Hono backend for Cloudflare Pages (functions/api/[[path]].js)
 *
 * - Robust, tolerant D1 handling (normalizes different .all() shapes)
 * - Slug support: product.slug column reads/writes; unique slug generation
 * - Detects presence of products.is_active and applies filter only if column exists
 * - Auth (login/logout) with JWT cookie (httpOnly). Uses JWT_SECRET env binding.
 * - Admin routes protected by cookie auth + role check
 * - Public store routes:
 *    GET  /store/products
 *    GET  /store/products/:id
 *    GET  /store/products/slug/:slug
 * - Admin product/category/stock/gallery CRUD endpoints
 * - Checkout + webhook placeholders (Paspay integration variables if present)
 * - Debug route /debug/db protected by DEBUG_TOKEN env var (temporary)
 *
 * Deploy this file to functions/api/[[path]].js in your Pages repo.
 */
...
function normalizeAllResult(raw) {
  // D1 driver sometimes returns { results: [...] } or { results: ..., meta: ... }
  // or returns array directly in some contexts. Normalize to array.
  if (!raw) return [];
  if (Array.isArray(raw)) return raw;
  if (raw && Array.isArray(raw.results)) return raw.results;
  // If the driver returned an object with a single 'row' or similar, try to be tolerant.
  return [];
}
...
// Readable error wrapper for DB ops
async function tryDb(fn) {
  try {
    return { ok: true, value: await fn() };
  } catch (e) {
    console.error('DB ERROR', e && (e.stack || e.message || e));
...
async function tableHasColumn(db, tableName, columnName) {
  try {
    ...
    return rows.some((r) => (r.name || r.column_name || '').toLowerCase() === columnName.toLowerCase());
  } catch (e) {
...
const authMiddleware = async (c, next) => {
  const env = c.env;
  // getCookie helper returns string value or undefined
  const token = getCookie(c, 'auth_token');
  if (!token) return c.json({ error: 'Tidak terotentikasi' }, 401);
  try {
    const payload = await verify(token, env.JWT_SECRET, 'HS256');
    const userRes = await env.DB.prepare('SELECT id, email, role, status FROM users WHERE id = ?').bind(payload.sub).first();
    if (!userRes) {
      // clear cookie
...
/* -------------------------
   ...
   registers path, /api/path and /api/api/path
   ------------------------- */
function register(method, path, ...handlers) {
  ...
  app[m]('/api/api' + path, ...handlers); // defensive fallback
}
...
async function loginHandler(c) {
  ...
  try {
    ...
    // verify password using PBKDF2 scheme matching how passwords were stored
    let verified = false;
    try {
      const [saltHex, hashHex] = (user.password_hash || '').split(':');
      if (saltHex && hashHex) {
        // Web Crypto based verification
        ...
        // fallback naive compare (if stored plaintext or other)
    ...
    // Detect dev vs prod for secure cookie flag
...
/* -------------------------
   Admin: products CRUD (with slug handling)
   ------------------------- */
...
async function adminUpdateProduct(c) {
  ...
  // generate slug if empty, ensure uniqueness except current id
  let slug = (body.slug || '').trim();
...
/* -------------------------
   Orders, Checkout & Webhook (placeholders)
   ------------------------- */

async function storeCheckoutHandler(c) {
  const env = c.env;
  // Using zValidator when registering will validate payload - here we assume correct shape
  const body = c.req.valid('json');
  try {
    ...
    // Example integration: call external Paspay if configured
    if (!env.PASPAY_API_URL || !env.PASPAY_API_KEY) {
      // Fallback: create local order row
    ...
    // Real integration omitted here — return not implemented
...
async function paspayWebhookHandler(c) {
  // simple verification by header token
  const env = c.env;
  ...
  // process payload according to paspay docs
