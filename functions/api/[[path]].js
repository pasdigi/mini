...
const verifyPassword = async (password, storedHash) => {
  try {
    ...
    console.error("Kesalahan saat verifikasi password:", e && e.message);
    return false;
  }
};
...
// --- Inisialisasi Hono ---
// IMPORTANT: Do NOT set basePath('/api') here when this file is located at functions/api/[[path]].js
// Cloudflare Pages will route requests to /api/* to this function; Hono should register routes exactly as the incoming pathname (i.e. '/admin/...' or '/store/...')
const app = new Hono();

// --- Debug Logging Middleware ---
// This logs incoming requests so you can see the real path being received by the function in your hosting logs.
app.use('*', async (c, next) => {
  try {
    ...
    console.log('[INCOMING] failed to read request url:', e && e.message);
  }
  await next();
});
...
app.post('/store/checkout', zValidator('json', checkoutSchema), async (c) => {
  ...
  try {
    ...
    if (product.product_type === 'UNIQUE') {
      ...
      if (!stock) {
        return c.json({ error: 'Stok produk ini telah habis!' }, 410);
      }
      ...
      if (changes === 0) {
        return c.json({ error: 'Stok produk baru saja habis. Silakan coba lagi.' }, 409);
      }
    }
...
// Webhook Paspay (public endpoint, token verified inside)
app.post('/webhook/paspay', zValidator('json', paspayWebhookSchema), async (c) => {
  ...
  try {
    ...
    console.error('Webhook Gagal: ' + e && e.message);
    return c.json({ error: 'Internal Server Error: ' + e.message }, 500);
  }
});

// --- Admin routes: mount under '/admin' ---
// Create a sub-router for admin to apply auth middleware cleanly
...
// Mount adminRouter under '/admin' so effective paths are '/admin/...'
app.route('/admin', adminRouter);

// --- Fallback for unmatched routes (useful for debugging) ---
app.all('*', (c) => {
  try {
    console.log('[NO MATCH] Unmatched route. method=', c.req.method, 'url=', c.req.url);
    try {
      const u = new URL(c.req.url);
    ...
    console.log('[NO MATCH] fallback logging failed', e && e.message);
  }
